...
def simple_search_chunks(db_path: str, keywords: List[str]) -> List[Dict]:
    ...
    conn.close()
    return [
        {'id': r[0], 'source_id': r[1], 'idx': r[2], 'text': r[3]}
        for r in rows
    ]
...
def evaluate_topic(db_path: str, keywords: List[str]) -> Dict:
    ...
    if not chunks:
        return {
            'ok': False,
            'message': 'Nenhuma evidência encontrada',
            'confidence': 0.0
        }
    ...
    confidence = min(
        0.99,
        0.1 + 0.9 * (top['count'] / (top['count'] + support_count))
    )
    ok = (confidence >= CONFIDENCE_THRESHOLD and support_count >= MIN_SUPPORT_SOURCES)
