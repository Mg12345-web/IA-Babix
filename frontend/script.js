// mantive seu script original, com envio de mensagens adaptado para API
...
const icon = sender === 'user' ? '<i class="fas fa-user"></i>' : '<i
...
return text.replace(/\n/g, '<br>').replace(/\*\*(.*?)\*\*/g,
'<strong>$1</strong>').replace(/\*(.*?)\*/g, '<em>$1</em>');
}
...
if(data.ok){
addMessage(`Confiança: ${data.confidence.toFixed(2)}\nFonte
principal: ${data.top.source}\n\n${data.top.sample}`, 'assistant');
} else {
13
addMessage(`Sem evidência suficiente automaticamente. \nSugestão: revisão
humana.`, 'assistant');
}
}catch(e){
removeTypingIndicator();
